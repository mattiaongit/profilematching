...
class PreProcessor():
	...
	def preprocess(self):
		...
		if self.priors == 0:
			profiles =[(profile['Alternion']['username'],[v['username'] for k,v in profile.items() if k != 'Alternion' and k in ('Facebook','Twitter','LinkedIn','Google+','Pinterest','Instagram') and len(v['username']) > 0]) for profile in list(self.rawdata)]
